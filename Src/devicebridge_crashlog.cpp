...
void DeviceBridge::SyncCrashlogs(QString path)
{
    AsyncManager::Get()->StartAsyncRequest([this, path]() {
        MobileOperation op = MobileOperation::CRASHLOG;
        QStringList id_mover = QStringList() << "com.apple.crashreportmover";
        QStringList id_afc = QStringList() << "com.apple.crashreportcopymobile";

        if (!CreateClient(op, id_mover, id_afc))
            return;

        lockdownd_service_descriptor_t service_mover = GetService(op, id_mover);
        if (!service_mover)
            return;
        ...
        service_error_t err = service_client_new(m_clients[op]->device, service_mover, &m_clients[op]->service);
        if (err != SERVICE_E_SUCCESS)
        {
            emit MessagesReceived(MessagesType::MSG_ERROR, "ERROR: Could not connect to " + id_mover.join(", ") + " client! " + QString::number(err));
            return;
        }
        ...
        while ((strncmp(ping, "ping", 4) != 0) && (attempts < 10)) {
            uint32_t bytes = 0;
            err = service_receive_with_timeout(m_clients[op]->service, ping, 4, &bytes, 2000);
        ...
        if (aerr != AFC_E_SUCCESS) {
            emit MessagesReceived(MessagesType::MSG_ERROR, "ERROR: Could not connect to " + id_afc.join(", ") + " client! " + QString::number(err));
            return;
        }

        QDir().mkpath(path);
        int result = afc_copy_crash_reports(m_clients[op]->afc, ".", path.toUtf8().data(), path.toUtf8().data());
        RemoveClient(op);
